...
function initialize() {
  ...
  //document.getElementById('a_misrutas').addEventListener('click',F_misrutas, false);
    ...
    $(".img_noti_class").click(function()
    {
      ...
      $.ajax({
        ...
        success: function(peticion){
          ...
          $("#label_notificacion").text(peticion.numero);
        },
        error: function(data){
         // console.log(data.responseText);
          swal({  title: 'Error!',   text: 'Error',   timer: 2000 });
    ...
    //$("#label_notificacion").text(misNotificaciones.length);
    document.getElementById('a_iniciar_ruta').addEventListener('click',F_iniciaruta, false);
    document.getElementById('a_misrutas').addEventListener('click',F_misrutas, false);
        ...
        //  console.log(seg.siguiendo);
          $.ajax({
            ...
            success: function(usuarios){
              ...
              eventos_presentacion('#seccion_siguiendo');
            },
            error: function(data){
              //console.log(data.responseText);
            ...
            success: function(usuarios){
              $.each(usuarios,function(i,usuario){
                ...
                if(usuario.username==seg.siguiendo){
                  user=usuario.first_name + " " + usuario.last_name;
                  crear_presentancion_usuario_seguidores('#seccion_seguidores', user,usuario.username)
                  //crear_presentancion_usuario('#seccion_seguidores', user,usuario.username, 'primary', "Seguir");
                }
              });
              eventos_presentacion('#seccion_seguidores');
            },
            error: function(data){
...
function F_buscar(evt) {
  ...
  cargarComponentes_Buscar('.otra_seccion');
...
function crear_presentancion_usuario_seguidores(seccion,nombre,id){
  $("<div>" ,{
    id : 'cuerpo_presentacion',
    class: 'cuerpo_presentacion_class'
  }).append($('<div>',{
...
function seguir(seguidor_a, seccion){
    var csrf =  $('input[name="csrfmiddlewaretoken"]').val();
    $.ajax({
      type : "POST",
      url:'/seguir',
      ...
      success: function(){
         if(seccion=='#seccion_buscar'){mostrar_busqueda();}
         //if(seccion=='#seccion_seguidores'){pruebaSegui();}
...
function dejar_de_seguir(labelText,seccion){
    ...
    $.ajax({
      ...
      success: function(){
         ...
         if(seccion=='#seccion_siguiendo'){getSiguiendos();}
         if(seccion=='#seccion_buscar'){mostrar_busqueda();}
         //if(seccion=='#seccion_seguidores'){pruebaSegui();}
