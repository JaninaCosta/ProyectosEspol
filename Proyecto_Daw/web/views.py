...
def obtenerTodasPeticiones_filtro(request):
    ...
    if request.method == 'GET':
        usuario= request.user
        peticiones_f = Peticion.objects.filter(fecha_pe=tiempo,fk_persona_peticion=usuario)
        #peticiones_m= peticiones_f.exclude(fk_persona_peticion=request.user)
        rutas= Ruta.objects.all()
        otrasRutas= rutas.exclude(fk_persona_ruta=usuario)
        list_peticion=list()
        for ruta in otrasRutas:
...
def misPeticiones (request):
    import time
    tiempo = time.strftime("%Y-%m-%d")
    if request.method == 'GET':
        usuario= request.user
        peticiones_p = Peticion.objects.filter(fecha_pe=tiempo,estado='Pendiente')
        rutas= Ruta.objects.filter(fk_persona_ruta=usuario)
        todasRutas=Ruta.objects.all()
        otrasRutas= todasRutas.exclude(fk_persona_ruta=usuario)
        list_peticion=list()
        for ruta in rutas:
            for p in peticiones_p:
                if(p.fk_pet_ruta==ruta):
                    list_peticion.append(p)


        peticiones_a = Peticion.objects.filter(fecha_pe=tiempo,estado='Aceptado',fk_persona_peticion=request.user)
        for ruta in otrasRutas:
            for p in peticiones_a:
                if(p.fk_pet_ruta==ruta):
                    list_peticion.append(p)

        peticiones_n = Peticion.objects.filter(fecha_pe=tiempo,estado='Negado',fk_persona_peticion=request.user)
        for ruta in otrasRutas:
            for p in peticiones_n:
                if(p.fk_pet_ruta==ruta):
                    list_peticion.append(p)


        rutas_tiempo = Ruta.objects.filter(fecha=tiempo)
        rutas_creadas= rutas_tiempo.exclude(fk_persona_ruta=request.user)

        totalPeticion = len(list_peticion)+len(rutas_creadas)
        response = render_to_response(
            'json/numPeticion.json',
            {'peticion': totalPeticion}
        )
        response['Content-Type'] = 'application/json; charset=utf-8'
        response['Cache-Control'] = 'no-cache'
        return response
...
def obtenerUsuariosSiguiendos(request):
    if request.method=='GET':
        listSiguiendos=list()
        listUsuariosSiguiendos=list()
        usuario= request.user
        if usuario.is_authenticated():
            listSiguiendos= Seguidor.objects.filter(fk_seguidor=usuario)
            for seg in listSiguiendos:
                listUsuariosSiguiendos.append(User.objects.filter(username=seg.fk_persona)[0])

            response = render_to_response(
                'json/users.json',
                {'users':listUsuariosSiguiendos},
                context_instance=RequestContext(request)
            )
            response['Content-Type'] = 'application/json; charset=utf-8'
            response['Cache-Control'] = 'no-cache'
...
def obtenerUsuariosSeguidores(request):
    if request.method=='GET':
        listSeguidores=list()
        listUsuariosSeguidores=list()
        usuario= request.user
        if usuario.is_authenticated():
            listSeguidores= Seguidor.objects.filter(fk_persona=usuario)
            for seg in listSeguidores:
                listUsuariosSeguidores.append(User.objects.filter(username=seg.fk_seguidor)[0])

            response = render_to_response(
                'json/users.json',
                {'users':listUsuariosSeguidores},
                context_instance=RequestContext(request)
            )
            response['Content-Type'] = 'application/json; charset=utf-8'
            response['Cache-Control'] = 'no-cache'
